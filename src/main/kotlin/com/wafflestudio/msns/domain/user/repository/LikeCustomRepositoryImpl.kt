...
class LikeCustomRepositoryImpl(
    ...
    override fun getLikes(
        ...
        val fetch = queryFactory
            .select(
                Projections.constructor(
                    LikeResponse.DetailResponse::class.java,
                    like
                )
            )
            .from(like)
            ...
            .where(
                post.id.eq(postId)
                    .and(
                        JPAExpressions
                            .selectFrom(block)
                            .where(block.fromUser.eq(loginUser).and(block.toUser.eq(like.user)))
                            .exists()
                            .not()
                    )
                    .and(
                        JPAExpressions
                            .selectFrom(block)
                            .where(block.fromUser.eq(like.user).and(block.toUser.eq(loginUser)))
                            .exists()
                            .not()
                    )
        ...
        val query = fetch.fetch()
            as MutableList<LikeResponse.DetailResponse>
